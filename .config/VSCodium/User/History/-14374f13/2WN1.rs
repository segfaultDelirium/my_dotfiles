...
fn validate_pesel(pesel: &str) -> (bool, &str){
    ...
    let error_message = "Checksums don't match, calculated checksum: {calculated_digit}, pesel checksum: {pesel_digit}";
    ...
    if m == 0 {
        ...
        let error_message = if are_matching {""} else {error_message};
        return (are_matching, error_message);
    }
    let are_matching = 10 - m == pesel_digit;
    return (are_matching, error_message);
}
