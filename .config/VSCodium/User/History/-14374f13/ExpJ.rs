...
fn validate_pesel(pesel: &str) -> (bool, &str){
    if pesel.len() != 11{
        return (false, "Pesel length is not 11.");
    }
    if !has_only_digits(pesel){
        return (false, "Pesel should only contain digits");
    }
    let calculated_digit = calculate_checksum_digit(pesel);

    let pesel_digit = pesel.chars().last().unwrap().to_string().parse::<i32>().unwrap();
    if calculated_digit == 0 {
        let are_matching = pesel_digit == 0;
        let error_message = if are_matching {""} else {"Checksums don't match, calculated checksum: {calculated_digit}, pesel checksum: {pesel_digit}"};
...
fn calculate_checksum_digit(pesel: &str) -> i32{
